...
void recover_disk_5(disk_array_t *da, int nblocks, int ndisks, int disk){
...
int main( int argc, char *argv[] )
{
  ...
  //Check for more the 1 disk for RAID 5
  if( level == 5){
    if(ndisks < 2){
      printf("ERROR: Must have at least 2 disks for RAID 5\n");
      return 1;
    }
  }
  ...
  while (!feof(fp)) 
	...
	\
	...
	else if(!strcmp(cmds[0],"RECOVER"))
	  {
	    ...
	    case 4:
	    case 5:
	      recover_disk_5(&da,nblocks,ndisks,atoi(cmds[1]));
	      break;
	    }
	  }
